...
class LayerAbstractCreator : BuildCreator<LayerDrawable> {
  ...
  fun padding(padding: Int) =
    apply { paddingLeft(padding).paddingTop(padding).paddingRight(padding).paddingBottom(padding) }

  @RequiresApi(Build.VERSION_CODES.M)
  fun paddingRelative(padding: Int) =
    apply { paddingStart(padding).paddingTop(padding).paddingEnd(padding).paddingBottom(padding) }
  ...
  inline fun build(block: LayerAbstractCreator.() -> Unit) {
    this.block()
    build()
  }
